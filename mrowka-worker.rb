...
while true
	...
	task = MrowkaTask.all.find{|t| t.status.state != 'done' && t.status.state != 'error'} # TODO all?
	sleep 5
	next if !task
	
	# if too many edits made in last 24 hours, stop.
	if MrowkaTask.all.select{|task| !task.touched || Time.now-task.touched < 24*60*60 }.map{|task| task.status.change_done }.compact.inject(0, :+) >= MrowkaConfig['worker']['dailylimit'].to_i
